...
const OngoingTask = ({navigation, reduxDeleteTask, reduxMarkTaskAsDone}) => {
  const todos = useSelector(state => state.taskReducer);
  //   console.log('completed task: ', todos);
  let i = 0;
  let taskArray = [];
  for (i in todos) {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.titleContainer}>
          <TouchableHighlight>
            <Text>You do not have any ongoing task</Text>
          </TouchableHighlight>
        </View>
      </View>
    );
    ...
    <View style={styles.container}>
      <View style={styles.taskCountContainer}>
        <Text style={{textAlign: 'right'}}>
          You have {taskArray.length} ongoing task
        </Text>
      </View>
      <SwipeListView
        data={taskArray}
...
export default connect(mapStateToProps, mapDispatchToProps)(OngoingTask);
